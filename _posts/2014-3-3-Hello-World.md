...
Let's have a look at the `SteamGameServer_InitEx` function which is used to initialize the GameServer API. It takes a bunch of arguments, the first being the IP, and the two following parameters being two ports. The first one is the "game server" port, the second one is the "query port". It's theoretically possible to combine these two and only use one port by specifying the `STEAMGAMESERVER_QUERY_PORT_SHARED` constant for the query port argument. This is going to change the way that Steam handles queries. Instead of Steam setting up a separate socket and monitoring that for incoming packets, the user will be responsible for separating incoming traffic on the socket, calling `HandleIncomingPacket` to handle it.
